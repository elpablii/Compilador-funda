...
{
       0,   105,   105,   116,   117,   127,   128,   129,   130,   131,
     132,   133,   134,   135,   139,   144,   145,   146,   150,   152,
     157,   162,   164,   169,   174,   179,   184,   188,   189,   190,
     191,   192,   193,   194,   198,   199,   200,   204,   205,   206,
     210,   214,   215,   216,   217,   225,   229
};
#endif
...
#line 118 "parser.y"
        {
            if ((yyvsp[0].node) != nullptr) { // Importante chequear si la sentencia produjo un nodo
...
#line 134 "parser.y"
                            { (yyval.node) = nullptr; /* Sentencia vacía, no crea nodo */ }
#line 1258 "parser.tab.c"
    break;

  case 13: /* statement: T_ERROR T_SEMICOLON  */
#line 135 "parser.y"
                            { yyerrok; (yyval.node) = nullptr; /* Recuperación de error simple */ }
...
#line 217 "parser.y"
                            {
                                ...
                                std::string s_val_str((yyvsp[0].sval)); // Crear std::string a partir de char*
                                free((yyvsp[0].sval)); // Liberar la memoria asignada por strndup/copy_yytext en el scanner
                                (yyval.node) = new StringLiteralNode(s_val_str);
                            }
...
#line 229 "parser.y"
                 {
                                ...
                                std::string id_name_str((yyvsp[0].sval)); // Crear std::string a partir de char*
                                free((yyvsp[0].sval)); // Liberar la memoria asignada por copy_yytext en el scanner
                                (yyval.id_node) = new IdentifierNode(id_name_str);
                            }
